...
import { Camera, ShieldCheck, Bell, Moon, Languages, LogOut, ChevronRight, Loader } from "lucide-react";
    ...
    <div className="flex flex-col h-full bg-slate-50/50 dark:bg-slate-900 w-full">
      <ScrollArea className="flex-1 w-full">
        <div className="w-full mx-auto py-6 md:py-10 px-4 md:px-6 space-y-8">
          ...
          <div className="bg-white dark:bg-slate-800 p-6 md:p-10 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col items-center text-center">
            ...
            <p className="text-slate-400 text-xs mt-2">@{user.id?.slice(0, 8)}</p>
          ...
          <div className="space-y-4">
            <h3 className="px-2 text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t.settings}</h3>
            
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
              <SettingItem icon={Bell} label={t.notifications_setting} description={language === 'vi' ? 'Âm thanh, tin nhắn đẩy' : 'Sound, push messages'} />
              <SettingItem icon={ShieldCheck} label={t.privacy} description={language === 'vi' ? 'Mật khẩu, khóa ứng dụng' : 'Password, app lock'} />
              
              <div className="flex items-center justify-between p-4 md:p-5 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all border-b border-slate-50 dark:border-slate-700 last:border-0 group">
                <div className="flex items-center gap-4">
                  <div className="p-2 bg-slate-100 dark:bg-slate-700 rounded-xl text-slate-600 dark:text-slate-300 group-hover:bg-slate-900 group-hover:text-white transition-colors">
                    <Moon className="w-5 h-5" />
                  </div>
                  <span className="font-semibold text-sm md:text-base text-slate-900 dark:text-white">{t.darkMode}</span>
                </div>
                <CustomSwitch checked={theme === 'dark'} onCheckedChange={(checked) => setTheme(checked ? 'dark' : 'light')} />
              </div>
              
              <div onClick={() => setShowLanguageDialog(true)} className="flex items-center justify-between p-4 md:p-5 hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer transition-all border-b border-slate-50 dark:border-slate-700 last:border-0 group">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-semibold text-sm md:text-base text-slate-900 dark:text-white leading-none">{t.language}</p>
                    <p className="text-[11px] md:text-xs text-slate-400 mt-1.5">{language === 'vi' ? 'Tiếng Việt' : 'English'}</p>
                  </div>
                </div>
          ...
          <button onClick={handleLogout} className="w-full p-4 md:p-5 flex items-center justify-center gap-3 text-red-500 font-bold bg-white dark:bg-slate-800 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl transition-all border border-red-100 dark:border-red-900/50 shadow-sm">
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-md bg-white dark:bg-slate-800">
          <DialogHeader>
            <DialogTitle className="dark:text-white">{t.editProfile}</DialogTitle>
          </DialogHeader>

          <div className="space-y-4">
            {/* Display Name */}
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-900 dark:text-slate-200">{t.displayName}</label>
              <Input
            ...
            {/* Bio */}
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-900 dark:text-slate-200">{t.bio}</label>
              <Textarea
                placeholder={language === 'vi' ? 'Kể về bạn...' : 'Tell us about yourself...'}
                value={bio}
  ...
  return (
    <div className="flex items-center justify-between p-4 md:p-5 hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer transition-all border-b border-slate-50 dark:border-slate-700 last:border-0 group">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="font-semibold text-sm md:text-base text-slate-900 dark:text-white leading-none">{label}</p>
